...
createOtherDivForSelect(htmlDropdown.parentElement);        // itt kell meghívni, mert eddig nem létezett ez a card, és a következő sorban meg már használjuk (így előtte kell létrehozni)
...
function hideBasedOnSelected(dropdownList){
    const listValue = dropdownList.value;   // parameter valueja (a selected elem)

    const parentDiv = dropdownList.parentElement;   // parameter parentElementje
    const cardClasses = parentDiv.querySelectorAll(".card");    // elkerjuk a div osszes card osztallyal rendelkezo elemet

    for (const card of cardClasses) {   // vegigiteralunk az osszes card osztalyos elemen
        card.classList.add("hide");     // es mindegyiknek megadjuk a hide osztalyt (azaz elrejtjuk)
    }

    makeVisibleBasedOnId(parentDiv, listValue); // majd meghivjuk ezt a fuggvenyt es lathato lesz ha a html-be az optionhoz beirjuk hogy selected
}
...
function hideBasedOnCheckbox(checkbox){
    const parentDiv = checkbox.parentElement;   // kirakjuk változóba a checkbox parentjét
    if (checkbox.checked) {     // megvizsgáljuk, hogy a checkboxunk be van e pipálva
        makeVisibleBasedOnId(parentDiv, 'ferfi');   // ha igen, megjelenitjük a férfi kártyát
        makeInvisibleBasedOnid(parentDiv, 'no');    // + eltüntetjük a nőt
    } else {
        makeVisibleBasedOnId(parentDiv, 'no');      // es ugyanez megcserelve ha nincs bepipalva
        makeInvisibleBasedOnid(parentDiv, 'ferfi');
    }
}
...
function changeDropdownList(e){
     ...
     */
    const dropdownList = e.target;  // dropdownList kiemelése valtozoba
    
    hideBasedOnSelected(dropdownList);  // meghivjuk ra eme fuggvenyunket
}
...
function changeCheckbox(e){
    const checkbox = e.target;  // checkbox kiemelése valtozoba

    hideBasedOnCheckbox(checkbox);  // meghivjuk a masik csodalatos fuggvenyt is
}